package net.chiappone.util.zip;
...
public class ZipUtil {
    ...
    private static void addZipEntry( ZipOutputStream zos, File unzipped ) throws FileNotFoundException, IOException {
        ...
        BufferedInputStream bis = new BufferedInputStream( new FileInputStream( unzipped ), BUFFER_SIZE );

        // Remove the absolute path so that we don't have the full path to the file in the zip
        String source = unzipped.toString()
                        .substring( unzipped.toString().lastIndexOf( "\\" ) + 1, unzipped.toString().length() );
